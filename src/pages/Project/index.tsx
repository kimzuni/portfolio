	...
	return (<>
		<Section className="md:mb-8">
			...
			{date && <ScrollFade className="flex justify-center flex-wrap gap-2 mb-8">
				<span>{date[0]}</span>
				<span>~</span>
				<span className="empty:hidden">{date[1]}</span>
			</ScrollFade>}
			{badges && skills &&
		...
		{sections.map(({ images=[], description, reverse }, idx) => {
			...
			return (
				<Section key={idx} className={`flex flex-col gap-10 mt-16 px-0 ${reverse ? "md:flex-row-reverse" : "md:flex-row"} md:px-(--section-px) md:items-center`}>
					{images.length !== 0 &&
					...
					{description &&
						<ScrollFade className={`flex-1 flex flex-col gap-6 mx-auto text-center px-(--section-px) leading-7 ${reverse ? "md:text-right" : "md:text-left"}`}>{description.map((x, i) => <p key={i}>{x}</p>)}</ScrollFade>
					}
				</Section>
			);
		})}
